  ...
  <div>
    ...
    <div v-if="pending" class="flex items-center justify-center py-12">
      <div class="text-center">
        ...
        <p class="mt-4 text-gray-600">Loading memory...</p>
      </div>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-6">
      <h2 class="text-lg font-semibold text-red-800 mb-2">Error Loading Memory</h2>
    ...
    <div v-else-if="!item" class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
      ...
      <p class="text-yellow-600">This memory does not have any data.</p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <UCard>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Last Accessed</label>
            <div class="flex items-center gap-2 text-gray-900">
      ...
      <UCard>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Description Length</label>
            <p class="text-gray-900">{{ item.description.length }} characters</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Estimated Tokens</label>
            <p class="text-gray-900">~{{ Math.ceil(item.description.length / 4) }} tokens</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Word Count</label>
            <p class="text-gray-900">{{ item.description.split(/\s+/).length }} words</p>
          </div>
