  ...
  <div>
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center gap-2 mt-4">
        ...
        <UButton to="/admin/learning/cases" variant="ghost" size="xs">Cases</UButton>
        <span class="text-gray-400">/</span>
    ...
    <div v-if="pending" class="flex items-center justify-center py-12">
      <div class="text-center">
        ...
        <p class="mt-4 text-gray-600">Loading case...</p>
      </div>
    </div>

    <!-- Error State -->
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-6">
      <h2 class="text-lg font-semibold text-red-800 mb-2">Error Loading Case</h2>
    ...
    <div v-else-if="!item" class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
      ...
      <p class="text-yellow-600">This case does not have any data.</p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <UCard>
        ...
        <div class="space-y-6">
          ...
          <!-- Customer Situation -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Customer Situation</label>
            <div class="px-4 py-3 bg-blue-50 rounded-lg border border-blue-200">
              <p class="text-gray-900 whitespace-pre-wrap">{{ item.customer_situation }}</p>
            </div>
          </div>
          ...
          <!-- Guidance Provided -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Guidance Provided</label>
            <div class="px-4 py-3 bg-green-50 rounded-lg border border-green-200">
              <p class="text-gray-900 whitespace-pre-wrap">{{ item.guidance_provided }}</p>
            </div>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="typeof item.outcome === 'object'" class="space-y-3">
            ...
            >
              ...
              <div v-if="typeof value === 'object'" class="bg-gray-50 rounded p-3">
          ...
          <div v-else class="bg-gray-50 rounded-lg p-4">
            <pre class="text-sm text-gray-900 overflow-x-auto">{{ JSON.stringify(item.outcome, null, 2) }}</pre>
          </div>
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Situation Length</label>
            <p class="text-gray-900">{{ item.customer_situation.length }} characters</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Guidance Length</label>
            <p class="text-gray-900">{{ item.guidance_provided.length }} characters</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Total Tokens (Est.)</label>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Has Outcome</label>
            <UBadge
