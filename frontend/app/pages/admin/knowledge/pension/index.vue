  ...
  <ClientOnly>
    <div class="space-y-6">
      ...
      <UAlert
        ...
        color="red"
        variant="solid"
        title="Failed to load pension knowledge"
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-800">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <UBadge v-if="item.subcategory" color="indigo" variant="subtle">
                    {{ item.subcategory }}
                  </UBadge>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm">
                  <UButton
                    size="xs"
                    @click="navigateTo(`/admin/knowledge/pension/${item.id}`)"
                  >
                    View
                  </UButton>
                </td>
    ...
    <template #fallback>
      <div class="flex items-center justify-center min-h-screen">
        <div class="text-center">
          ...
          <p class="text-gray-500">Loading pension knowledge...</p>
        </div>
      </div>
    </template>
  </ClientOnly>
...
// Fetch data
const { data: apiData, pending, error, refresh } = await useFetch('/api/admin/pension-knowledge', {
  query: queryParams,
  headers: {
...
// Get unique subcategories for filter (filtered by category)
const subcategoryOptions = computed(() => {
  if (!apiData.value?.items || !filters.value.category) return []
  const subcategories = new Set(
    apiData.value.items
      .filter((item: any) => item.category === filters.value.category)
