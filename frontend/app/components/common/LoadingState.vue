<template>
  <div class="loading-state">
    ...
    <div v-if="variant === 'spinner'" data-testid="spinner" class="flex flex-col items-center justify-center p-12">
      <UIcon name="i-heroicons-arrow-path" class="w-12 h-12 text-primary-600 dark:text-primary-400 animate-spin" />
      <p v-if="message" class="mt-4 text-gray-600 dark:text-gray-400">{{ message }}</p>
    </div>
    ...
    <div v-else-if="variant === 'overlay'" data-testid="overlay" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-8 shadow-xl">
        <UIcon name="i-heroicons-arrow-path" class="w-12 h-12 text-primary-600 dark:text-primary-400 animate-spin mx-auto" />
        <p v-if="message" class="mt-4 text-gray-900 dark:text-gray-100">{{ message }}</p>
      </div>
    </div>
  </div>
</template>
